...
class ShogiController @Inject() (cc: ControllerComponents)(implicit
  ...
  extends AbstractController(cc) with I18nSupport with Publisher {
  ...
  def socket: WebSocket = WebSocket.accept[String, String] { _ =>
    ActorFlow.actorRef { out =>
      ...
      ShogiWebSocketActorFactory.create(out, this)
    }
  }

  object ShogiWebSocketActorFactory {
    def create(out: ActorRef, shogi: ShogiController): Props = {
      Props(new ShogiWebSocketActor(out, shogi))
    }
  }

  class ShogiWebSocketActor(out: ActorRef, shogi: ShogiController) extends Actor with Reactor {
    listenTo(shogi)
